...
xValues <- seq(-5, 5, by = 0.1) #0.1 is increments*



# calculate y-values from the functions f_1 and f_2 # initial, 1.0

y_f1 <- f1(x = xValues, mu = 0.0, sigma = 1.0)

y_f2 <- f2(x = xValues, tau = 1.0, mu = 0)


# calculate y-values from the functions f_1 and f_2 # 2.0

y_f1 <- f1(x = xValues, mu = 0.0, sigma = 3.0)

y_f2 <- f2(x = xValues, tau = 3.0, mu = 0)
...
# checking that f1 and f2 are he same - when both sigma, tau = 1
library(tidyverse)
...
## new parameters # already added above
...
## Higher sigma --> higher standard deviation --> wider curve. Lower sigma opposite.
...
print(paste("The mean of the total number of shots is", meanThrows_basketball, "and the std of total number of shots is", sdThrows_basketball))
...
# for loop making the 20 x-values, adding each to a list
for (i in 1:20)
{
  singleX <- runif(1, 0, 1) # uniform distribution, number of random numbers to be generated (1), going from min (0) to max (1)
  combinedX <- append(combinedX, singleX) # logs the new singleX
  
}

# sums together the values of the list, aka the 20 xValues
sum_of_combinedX <- sum(unlist(combinedX))
...
## create 1000 simulations of x and plot their histograms


#makes an empty list
combinedX_simulation <- list()
xValues_simulation <- list()


# looping the previous for loop 1000 times

for (j in 1:1000)
{
  combinedX_simulation <- c()
  
  
 for (i in 1:20)
 {
  singleX <- runif(1, 0, 1) # uniform distribution, number of random numbers to be generated (1), going from min (0) to max (1)
  combinedX_simulation <- c(combinedX_simulation, singleX) # logs the new singleX
 }
  
 # sums together the values of the list, aka the 20 xValues
 sum_of_combinedX_simulation <- sum(combinedX_simulation)
 
 xValues_simulation <- append(xValues_simulation, sum_of_combinedX_simulation)
 xValues_simulation
 
}

# as a list of lists have been made i unlist it, to have it be a numeric list
xValues_simulation <- unlist(xValues_simulation)
...
# making a plot
ggplot(data.frame(x = xValues_simulation), aes(x)) + 
  geom_histogram(aes(y = ..density..), bins = 30, fill = "skyblue", color = "black") +
  stat_function(fun = dnorm, args = list(mean = mean(xValues_simulation), sd = sd(xValues_simulation)), color = "red") +
...
### from pernille's solution - giving about the same result as for my own code below
...
```{r myVersion complied with some chatGPT help}
...
for (i in 1:n_simulations) {
  # Generate samples of men's and women's heights
  men_heights <- rnorm(n_samples, mean = mean_men, sd = sd_men)
  women_heights <- rnorm(n_samples, mean = mean_women, sd = sd_women)
  
  # Calculate the average height for each sample
  avg_height_men <- mean(men_heights)
  avg_height_women <- mean(women_heights)
  
  # Calculate the difference between the average heights
  differences[i] <- avg_height_men - avg_height_women
}

# Plot the histogram of the differences
hist(differences, breaks = 30, main = "Distribution of Differences in Average Heights", xlab = "Difference in Average Heights (inches)", ylab = "Frequency")
...
cat("Standard deviation of difference:", sd_difference, "\n")
...
expectedTrueEffect <- 0.1
expectedIncrease <- 0.1
assumedSE <- 0.17


log_normal_values <- rlnorm(n = 1000, meanlog = 0.1)
...
for (i in 1000)
{
  normal_values <- rnorm(127, 0.1, 0.17)
  
  
  simulationData <- c(simulationData, normal_values)
...
simulationData <- unlist(simulationData)


## calculates the confidence intervals via a t-test

results <- t.test(simulationData)

confidenceIntervals <- results$conf.int
...
withinCI_count <- 0
TrueParameter_withinCI_count <- 0
expectedTrueEffect <- 0.1
...
for (i in 1:1000)
{
  ...
  results <- t.test(normal_values)

  confidenceIntervals <- results$conf.int
  
  # get the start and end of the CI
  start_CI <- confidenceIntervals[1]
  
  end_CI <- confidenceIntervals[2]
  
  
  # checks for each simulation if the value in within the confidence intervals # not needed - was just how i understood the task at first
  
  for (z in 1:127)
  {
      if(normal_values[z] >= start_CI && normal_values[z] <= end_CI)
    {
     withinCI_count <- withinCI_count + 1
  ...
  # checks for each simulation if the true parameter value is within the confidence intervals # this was the assignment
  for (z in 1:127)
  {
      if(expectedTrueEffect >= start_CI && expectedTrueEffect <= end_CI)
    {
     TrueParameter_withinCI_count <- TrueParameter_withinCI_count + 1
...
TrueParameter_withinCI_count # this stores the amount of times the true parameter value is within the CI for the simulations
...
for (i in 1:1000)
{
  ...
  ## calculates the p-values for each simulation -- checks for each simulation if the value is significantly different from 0

  results <- t.test(normal_values, mu = 0) # tests if normal_values is significantly different from 0 by a one-sample t-test


  if(results$p.value < 0.05)
  {
   count_significantlyDiff <- count_significantlyDiff + 1
...
count_significantlyDiff



### PART 2: Compute the average and standard deviation of the selected point estimates. Compare these to the result from Exercise 5.8.


#   **difference between this and solution is that the solution tries to previous tasks
...
is_prime <- function(number)
{
  ...
  # runs untill it has been determined whether a number is a prime or not
  while(run == TRUE)
  {
   ...
   if(number > 0 && trunc(number) == number)
   {
    ...
    {
         # for finding non-primes by checking if the input parameter is positive, divisible by a smaller number without a remainder, and divided by itself
    ...
    # if not a non-prime, then a prime
     if(prime == TRUE)
     {
       run = FALSE
     }
   ...
   {
    # if it didn't live up to the initial conditions of a prime, then it isn't one
