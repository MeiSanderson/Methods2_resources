...
myFunc <- function(x)
  {
   func_x <- x^4 + x^3 - 5*x^2 # my function w. 2 minima
   return(func_x)
  }
...
```
  ...
  b) Find the 2-4 minimums using the optim() function.
```{r}

firstMinimum <- optim(par = -3, fn = myFunc, method = "CG")

firstMinimum

print("PAUSE")

secondMinimum <- optim(par = firstMinimum$par + 0.05, fn = myFunc, method = "Brent", lower = firstMinimum$par+ 0.05, upper = 2)
secondMinimum
...
# testing if firstMinimum is in fact a minimum
func_x <- expression(x^4 + x^3 - 5*x^2) # my function w. 2 minima

firstMinimum_secondDerivativeT <- D( D(func_x, "x"), "x")


func_x1 <- function(x) x^4 + x^3 - 5*x^2
